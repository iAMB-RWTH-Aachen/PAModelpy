...
class PAMValidator(object):
    ...
    MW_ACETATE = 59.04
    MW_CO2 = 44.009
    GLC_GRADIENT_MAX = 110 #mmol/gdw/h
    GLC_GRADIENT_STEP = 5 #mmol/gdw/h
    ...
    def validate_range(self, pfba: bool = False, show: bool = True, save: bool = False):
            ...
            try:
                ...
                pass
            self.run_simulations_glc_gradient(pfba)
            jupyter = self.check_kernel_type()
            ...
            if show:
                ...
                if self._additional_rxns != []:
                    self.custom_plot(self._additional_rxns)
            if save:
    ...
    def parse_data(self):
        self.physiology_data = pd.melt(self.physiology_data, id_vars='Reaction_ID', var_name='Reference',
                                       value_vars=['Rijsewijk_2011', 'Nanchen_2006_1', 'Nanchen_2006_2', 'Nanchen_2006_3', 'Nanchen_2006_4'])
            ...
            for glc_flux in pd.unique(df.Glucose_IN):
                ...
                if i == 2:
                    cobra.core.solution.get_solution(self.model).fluxes.to_csv(
                        os.path.join(os.path.split(os.getcwd())[0], 'Results', 'test_out_mu.tsv'), sep='\t')
                i+=1

    def run_simulations_glc_gradient(self, pfba: bool = False):
        #run simulations for increasing glucose consumption rates
        print('Running simulations to validate the model...\n')
        self.model.reset_objective()
        for glc_flux in range(0, self.GLC_GRADIENT_MAX, self.GLC_GRADIENT_STEP):
            self.model.set_glc_uptake_bounds(glc_flux/10)
            if pfba: self.model.reset_objective()
            self.model.optimize()
            if self.model.solver.status == 'optimal':
    ...
    def run_simulations_glc_o2_gradient(self, oxygen_gradient: list, params_to_save:Union[str, list] = 'R_TranslationalProteinSector'):
        ...
        :param
            oxygen_gradient: list
                List of upperbounds for the oxygen uptake reaction to loop over
            params_to_save: optional, string or list
                which parameter(s) to save for further analysis (default: translational protein sector constraint)
        :return:
            results: list of dataframes
                saves the growth rate, glucose uptake rate and the user defined parameters for each oxygen uptake rate
                separate dataframes
        """
        print('Running simulations of glucose and oxygen gradients...\n')
        result = list()
        if isinstance(params_to_save,str): params_to_save = [params_to_save]
        rxns_to_save = [self.BIOMASS_REACTION, self.GLUCOSE_EXCHANGE_RXNID, self.OXYGEN_UPTAKE_RXNID] + params_to_save
    ...
    def run_simulations_ups(self, ups_gradient: list, params_to_save:Union[str, list] = 'R_TranslationalProteinSector'):
        """
        Function to run simulations with increasing unused enzyme sectors proportions for a range of growth rates
        This will simulate growth for the entire range of glucose concentrations for a range of fractions of ups_0 as
        given by the input.
        :param:
            ups_gradient: list
                List of upperbounds for the oxygen uptake reaction to loop over
    ...
    def set_rxn_uptake_bounds(self, rxn_id:str, bound:int,
                              upper:bool = True, lower:bool = False):
        rxn = self.model.reactions.get_by_id(rxn_id)
        if upper: rxn._upper_bound = bound
        if lower: rxn._lower_bound = bound
        rxn.update_variable_bounds()

    def store_results(self):
        result = {}
        glc_upt_flux = self.model.reactions.get_by_id(self.GLUCOSE_EXCHANGE_RXNID).flux
        #store the relevant flux values
        for id in self.PHYS_RXN_IDS:
            #getting reaction or variable
            try:result[id]=self.model.reactions.get_by_id(id).flux
            except:result[id]=self.model.variables[id].primal
            #calculate yields based on glucose for exchange reactions
            if ('EX' in id or 'BIOMASS' in id) and 'glc' not in id and '_o2' not in id and abs(glc_upt_flux)> 0:
                if 'BIOMASS' in id:
                    id_ext = 'biomass'
                    MW = 1
                if 'ac' in id:
                ...
                yield_id = 'Y_'+ id_ext
                result[yield_id]= (self.model.reactions.get_by_id(id).flux * MW) / (glc_upt_flux * self.MW_GLC * 1e-3)
    ...
    def store_manual_results(self, vars_to_save:list, df:pd.DataFrame):
        ...
        # appending the results to the dataframe
        df = pd.concat([df, pd.DataFrame({key: pd.Series(value) for key, value in row.items()})], ignore_index=True)
        return df
    ...
    def plot_mu(self):
        ...
        subplot_titles= ('Growth rate', 'Biomass yield', 'CO2 excretion', 'CO2 yield', 'Acetate excretion', 'Acetate yield'))
        result = self.results.__deepcopy__()
        result = result.merge(self.yield_data, left_on=self.GLUCOSE_EXCHANGE_RXNID, right_on='Glucose_IN')
        #print(tabulate(result, headers='keys', tablefmt='psql'))
        # growth rate
        fig.add_trace(go.Scatter(x=result[self.BIOMASS_REACTION], y=result['mu'], mode='markers', showlegend=False, marker=dict(color='blue')), row=1, col=1)
        fig.add_trace(go.Scatter(x=[0,0.5,1.4],y=[0,0.5,1.4], mode='lines', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)),row=1, col=1)
        fig.update_xaxes(title_text='Simulated growth rate [1/h]',row=1, col=1)
        fig.update_yaxes(title_text='Experimentally determined growth rate [1/h]',row=1, col=1)

        # biomass yield
        fig.add_trace(go.Scatter(x=result['Y_biomass'], y=result['Yield_biomass'], mode='markers',showlegend=False, marker=dict(color='blue')), row=1, col=2)
        fig.add_trace(go.Scatter(x=[0, 0.5, 0.6], y=[0, 0.5, 0.6], mode='lines',
                                 showlegend=False,
                                 line=dict(color='black', dash='dash', width =1)), row=1, col=2)
        fig.update_xaxes(title_text='Simulated biomass yield [g_x/g_glc]', row=1, col=2)
        fig.update_yaxes(title_text='Experimentally determined biomass yield [g_x/g_glc]', row=1, col=2)

        # CO2 yield
        fig.add_trace(go.Scatter(x=result['Y_co2'], y=result['Yield_co2'], mode='markers', showlegend=False, marker=dict(color='blue')), row=2, col=2)
        fig.add_trace(go.Scatter(x=[0, 15, 30], y=[0, 15, 30], mode='lines', showlegend=False,
                                 line=dict(color='black', dash='dash', width =1)), row=2, col=2)
        fig.update_xaxes(title_text='Simulated CO2 yield [mmol_x/g_glc]', row=2, col=2)
        fig.update_yaxes(title_text='Experimentally determined CO2 yield [mmol_co2/g_glc]', row=2, col=2)

        # CO2 out
        fig.add_trace(go.Scatter(x=result['EX_co2_e_f'], y=result['CO2_OUT'], mode='markers', showlegend=False,
                                 marker=dict(color='blue')), row=2, col=1)
        fig.add_trace(go.Scatter(x=[0, 15, 30], y=[0, 15, 30], mode='lines', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)), row=2, col=1)
        fig.update_xaxes(title_text='Simulated CO2 exchange rate [mmol/g_cdw/h]', row=2, col=1)
        fig.update_yaxes(title_text='Experimentally determined CO2 exchange rate [mmol/g_cdw/h]', row=2, col=1)
    ...
    def plot_vs_mu(self):
        ...
        self.results[self.GLUCOSE_EXCHANGE_RXNID] = abs(self.results[self.GLUCOSE_EXCHANGE_RXNID])
        self.results[configuration.OXYGEN_UPTAKE_RXNID] = abs(self.results[configuration.OXYGEN_UPTAKE_RXNID])

        #glucose consumption
        fig.add_trace(go.Scatter(x=self.results[self.GLUCOSE_EXCHANGE_RXNID], y=self.results[self.BIOMASS_REACTION],
                                 showlegend=False,  mode='lines', marker=dict(color='blue')), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.yield_data['Glucose_IN'],
                                 y=self.yield_data['mu'], mode='markers', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)),row=1, col=1)
        ...
        #acetate excretion
        fig.add_trace(go.Scatter(x=self.results[self.GLUCOSE_EXCHANGE_RXNID], y=self.results[configuration.ACETATE_EXCRETION_RXNID],
                                 showlegend= False, mode='lines', marker=dict(color='blue')), row=1, col=2)
        fig.add_trace(go.Scatter(x=self.yield_data['Glucose_IN'],
                                 y=self.yield_data['Acetate_OUT'],
                                 mode='markers', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)), row=1, col=2)
        fig.update_xaxes(title_text='Glucose consumption [mmol/gdw/h]', row=1, col=2)
        fig.update_yaxes(title_text='Acetate production rate [mmol/gdw/h]', row=1, col=2)

        # O2 uptake
        fig.add_trace(
            go.Scatter(x=self.results[self.GLUCOSE_EXCHANGE_RXNID], y=self.results[configuration.OXYGEN_UPTAKE_RXNID], mode='lines',
                       showlegend=False, marker=dict(color='blue')), row=2, col=1)
        fig.add_trace(go.Scatter(x=self.yield_data['Glucose_IN'],
                                 y=self.yield_data['O2_IN'],
                                 mode='markers', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)), row=2, col=1)
        ...
        #CO2 excretion
        fig.add_trace(go.Scatter(x=self.results[self.GLUCOSE_EXCHANGE_RXNID], y=self.results[configuration.CO2_EXHANGE_RXNID], mode='lines',
                                 showlegend=False, marker=dict(color='blue')), row=2, col=2)
        fig.add_trace(go.Scatter(x=self.yield_data['Glucose_IN'],
                                 y=self.yield_data['CO2_OUT'],
                                 mode='markers', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)), row=2, col=2)
    ...
    def custom_plot(self, rxn_ids:list, valid_dataframe: pd.DataFrame = None, xaxis:str = None):
        """
        Function to plot the results of custom reactions
        Parameters
        ----------
        rxn_ids: list of rxn_ids (str)
            reaction identifiers of the reactions to be plotted
        valid_dataframe: pandas.DataFrame, optional
            A dataframe with experimental data to validate the results with. The columns should be
             the same as the rxn_id of the reaction to be plotted and the reaction which should be
             plotted on the x-axis (by default the glucose exchange reaction 'EX_glc__D_e_b').
             If the dataframe is not provided, only the simulation results will be plotted.
        xaxis: str, optional
            the reaction identifier of the reaction which should be plotted on the x-axis
            (default: 'EX_glc__D_e_b')

        Returns
        -------
        prints scatter plots of the model simulations vs experimental datapoints (if provided)
        """
        if xaxis is None: xaxis = self.GLUCOSE_EXCHANGE_RXNID
        fig = make_subplots(
            rows=1, cols=len(rxn_ids),
            specs=[[{"type": "scatter"}]*len(rxn_ids)],
            subplot_titles=tuple(rxn_id[0] for rxn_id in rxn_ids))
        for i, rxn in enumerate(rxn_ids):
            fig.add_trace(
                go.Scatter(x=self.results[xaxis], y=self.results[rxn[0]], mode='lines',
                           showlegend=False, marker=dict(color='blue')), row=1, col=i+1)
            if valid_dataframe is not None:
                fig.add_trace(go.Scatter(x=valid_dataframe[xaxis],
                                        y=valid_dataframe[rxn],
                                        mode='markers', showlegend=False,
                                        line=dict(color='black', dash='dash', width=1)), row=1, col=i+1)
