...
class EnzymeSector(Sector):
    ...
    def add_sector(self, model, slope, intersect):
        self.model = model
        if self.mol_mass is None:
            self.mol_mass = [self.DEFAULT_MOL_MASS]

        lin_rxn = model.reactions.get_by_id(self.id_list[0])

        totprot, tpc_metabolite = self.get_tpc_metabolite(model)

        # constraint corresponding to the enzyme sector
        constraint = Constraint(self.id, compartment='sector')
        model.add_sector_constraints([constraint])

        # Protein concentration allocated to enzymes required for translation
        var = model.problem.Variable('R_' + self.id)
        model.add_cons_vars(var)

        # For translational protein at zero growth
        var_0 = Constant('R_' + self.id + '_0', coefficient=intersect, direction='-',
                                constraint=constraint)
        var_0.model = model  # the setter will add the variable to the model

        # link flux to enzyme concentration
        model.constraints[constraint.id].set_linear_coefficients({
            var: 1, var_0.forward_variable: var_0.coeff,
        ...
        if totprot:
            ...
            model.constraints[self.TOTAL_PROTEIN_CONSTRAINT_ID].set_linear_coefficients({
                var: -self.mol_mass[0] * 1e-6
            })

        # save reaction for easy removal of the sector
        self.variables += [var, var_0]
        return model
...
class TransEnzymeSector(EnzymeSector):
    ...
    BIOMASS_RXNID = 'BIOMASS_Ec_iML1515_WT_75p37M'
    ...
    def __init__(self, tps_0, tps_mu,id_list= [BIOMASS_RXNID], mol_mass=None):
        ...
        # *1000 to convert units from g/g_cdw to mg/g_cdw
        # /MW to convert to mg to mmol (which is used in the model)
        self.tps_0_coeff = self.tps_0[0] * 1000 / (self.mol_mass[0] * 1e-6)
        self.slope = None
        self.set_slope()
        self.intercept = None
        self.set_intercept()
    ...
    def add(self, model):
        ...
        return self.add_sector(model= model, slope = self.tps_mu[0]* 1e3 / (self.mol_mass[0] * 1e-6), intersect=self.tps_0_coeff)
    ...
    def set_slope(self):
        self.slope = self.tps_mu[0] * 1e3 / (self.mol_mass[0] * 1e-6)
...
class UnusedEnzymeSector(EnzymeSector):
    ...
    def __init__(self, id_list, s_max_uptake, ups_0, mol_mass=None):
        super().__init__(id_list, mol_mass)
        if self.mol_mass is None:
            self.mol_mass = [self.DEFAULT_MOL_MASS]
        ...
        self.s_max_uptake = s_max_uptake #maximal substrate uptake rate (mmol_s/g_cdw/h)
        self.ups_0 = ups_0 #amount of protein allocated to the excess enzyme sector at zero substrate uptake
        self.ups_mu = self.ups_0[0]/self.s_max_uptake[0] #slope of linear relation with growth
        self.id = 'UnusedEnzymeSector'
        self.ups_0_coeff = self.ups_0[0] * 1000/(self.mol_mass[0])

        self.slope = self.ups_mu*1000/(self.mol_mass[0])
    ...
    def add_to_model(self, model):
        ...
        if self.mol_mass is None:
            self.mol_mass = [self.DEFAULT_MOL_MASS]
        return self.add_sector(model= model, slope = self.ups_mu*1000/(self.mol_mass[0]), intersect=self.ups_0_coeff)
...
class CustomSector(EnzymeSector):
    ...
    def __init__(self, id_list, name,lin_rxn_id, cps_0,cps_s, mol_mass=None):
        ...
        self.cps_0_coeff = self.cps_0[0] * 1000 / (self.mol_mass[0])
        self.id = f'CustomProteinSector_{self.name}'

        self.slope = self.cps_mu[0]* 1000 / (self.mol_mass[0])
    ...
    def add_to_model(self, model):
        ...
        return self.add_sector(model= model, slope = self.cps_mu[0]* 1000 / (self.mol_mass[0]), intersect=self.cps_0_coeff)
