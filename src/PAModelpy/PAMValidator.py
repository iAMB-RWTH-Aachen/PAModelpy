...
class PAMValidator(object):
    ...
    def validate_range(self, c_uptake_rxn: str = GLUCOSE_EXCHANGE_RXNID, pfba: bool = False, show: bool = True, save: bool = False):
    ...
    def run_simulations_gradient(self, pfba: bool = False, c_uptake_rxn: str = GLUCOSE_EXCHANGE_RXNID):
    ...
    def set_rxn_uptake_bounds(self, rxn_id:str, bound:int,
        ...
        if rxn_id[-1] == 'b':
            ...
            if upper: rxn._lower_bound = -bound
            if lower: rxn._upper_bound = -bound
    ...
    def store_results(self, c_uptake_rxn: str = GLUCOSE_EXCHANGE_RXNID):
        ...
        for id in self.PHYS_RXN_IDS:
            ...
            if ('EX' in id or 'BIOMASS' in id) and 'glc' not in id and '_o2' not in id and abs(c_upt_flux)> 0:
                if 'BIOMASS' in id:
                    id_ext = 'biomass'
                    MW = 1
                if 'ac' in id:
                ...
                yield_id = 'Y_'+ id_ext
                result[yield_id]= (self.model.reactions.get_by_id(id).flux * MW) / (c_upt_flux * self.MW_GLC * 1e-3)
    ...
    def plot_mu(self):
        ...
        # CO2 out
        fig.add_trace(go.Scatter(x=result[self.CO2_EXHANGE_RXNID], y=result['CO2_OUT'], mode='markers', showlegend=False,
    ...
    def plot_vs_mu(self, c_uptake_rxn:str = GLUCOSE_EXCHANGE_RXNID):
        ...
        #glucose consumption
        fig.add_trace(go.Scatter(x=self.results[c_uptake_rxn], y=self.results[self.BIOMASS_REACTION],
                                 showlegend=False,  mode='lines', marker=dict(color='blue')), row=1, col=1)
        fig.add_trace(go.Scatter(x=self.yield_data[c_uptake_rxn],
                                 y=self.yield_data[self.BIOMASS_REACTION], mode='markers', showlegend=False,
                                 line=dict(color='black', dash='dash', width=1)),row=1, col=1)
    ...
    def custom_plot(self, rxn_ids:list, valid_dataframe: pd.DataFrame = None, xaxis:str = None, c_uptake_rxn:str = GLUCOSE_EXCHANGE_RXNID):
        ...
        """
        if xaxis is None: xaxis = c_uptake_rxn
        fig = make_subplots(
