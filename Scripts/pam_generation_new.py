...
def set_up_ecolicore_pam(total_protein:bool = True, active_enzymes: bool = True, translational_enzymes:bool = True, unused_enzymes:bool = True, sensitivity =True):
    ...
    if active_enzymes:
        ...
        rxn2protein_new = []
        for i in list(rxn2protein.keys())[:5]:
            print(i)
            rxn2protein_new[i] = rxn2protein[i].copy()

        rxn2protein = rxn2protein_new
...
if __name__ == '__main__':
    ecoli_pam = set_up_ecolicore_pam(sensitivity=False)
    ...
    for enzyme in ecoli_pam.enzymes:
        if hasattr(enzyme, 'enzymes'):
            print(enzyme)
            for c in enzyme._constraints.values():
                print(c)
    for ce in ecoli_pam.catalytic_events.query('AKGDH'):
        for c in ce.constraints.values():
            print(c)
