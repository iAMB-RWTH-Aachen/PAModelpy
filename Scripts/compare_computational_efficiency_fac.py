...
RESULT_DIR = RESULT_FILE = os.path.join(os.path.split(os.getcwd())[0], 'Results')
RESULT_RESOURCE_FILE = os.path.join(RESULT_DIR, 'toy-core-full_model_performance_fac.xls')
RESULT_FAC_FILE = os.path.join(RESULT_DIR, 'toy-core-full_model_FAC.xls')
...
for model in [toy_model_pam, ecolicore_pam]:#,
    ...
    for fac in ['rxn', 'enzyme', 'sector']:
        if fac == 'rxn':
            x_axis_fac += model.flux_allocation_coefficients[model.flux_allocation_coefficients['constraint'] == fac].rxn_id.to_list()
        else:
            x_axis_fac += model.flux_allocation_coefficients[
                model.flux_allocation_coefficients['constraint'] == fac].enzyme_id.to_list()

    df_coeff = pd.DataFrame(columns=['method']+x_axis_fac)
    #fcc
    time_start = time.perf_counter()
    fcc_fac = fcc_numeric_fac_optimizations(model)
    time_elapsed = (time.perf_counter() - time_start)
    res_used_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0 / 1024.0
    accuracy = sum(fcc_fac)
    resource_time_dataframe.loc[len(resource_time_dataframe)] = [model.id, 'fcc',
                                                                 accuracy, time_elapsed, res_used_mb]
    df_coeff.loc[len(df_coeff)] = ['FCC']+fcc_fac
    print(f'Done with calculatibng Flux Control Coefficients for the {model}.\nTime:\t\t {time_elapsed} seconds \nSum:\t\t{accuracy}')


    # #forward
    # time_start = time.perf_counter()
    # ffn_fac = first_forward_numeric_fac_optimizations(model)
    # time_elapsed = (time.perf_counter() - time_start)
    # res_used_mb =resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024.0/1024.0
    # accuracy = 1/sum(ffn_fac)
    # resource_time_dataframe.loc[len(resource_time_dataframe)] = [model.id, 'first order forward finite difference',
    #                                                              accuracy, time_elapsed, res_used_mb]
    # df_coeff.loc[len(df_coeff)] = ['Forward'] + fcc_fac
    # print(f'Done with calculating first order forward finite difference for the {model}.\nTime:\t\t {time_elapsed} seconds \nAccuracy:\t\t{accuracy}')

    #central
    time_start = time.perf_counter()
    model.optimize()
    fcn_fac = first_central_numeric_fac_optimizations(model)
    time_elapsed = (time.perf_counter() - time_start)
    res_used_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0 / 1024.0
    accuracy = sum(fcn_fac)
    resource_time_dataframe.loc[len(resource_time_dataframe)] = [model.id, 'first order central finite difference',
                                                                 accuracy, time_elapsed, res_used_mb]
    df_coeff.loc[len(df_coeff)] = ['central'] + fcc_fac
    ...
    time_start = time.perf_counter()
    model.optimize()
    Ccac,Cfac = parse_coefficients(model)
    time_elapsed = (time.perf_counter() - time_start)
    res_used_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024.0 / 1024.0
    accuracy = sum(Cfac)
    resource_time_dataframe.loc[len(resource_time_dataframe)] = [model.id, 'allocation coefficients',
                                                                 accuracy,time_elapsed, res_used_mb]
    df_coeff.loc[len(df_coeff)] = ['allocation'] + fcc_fac
    print(f'Done with calculating Flux Allocation Coefficients for {model}.\nTime:\t\t {time_elapsed} seconds \nSum:\t\t{accuracy}\n')

    coeff_dict[model.id] = df_coeff

    try:
        print_heatmap(x_axis_fac, [fcc_fac, fcn_fac, Cfac], yaxis=['fcc', 'central', 'allocation'])
    except:
        continue
...
fcc_fac = fcc_numeric_fac_calculation(ecoli_pam, dummy_rxn, f'{dummy_rxn.id}_ub', obj)
...
time_start = time.perf_counter()
ecoli_pam.optimize()
fcn_fac = first_central_numeric_fac_calculation(ecoli_pam, dummy_rxn, f'{dummy_rxn.id}_ub', obj)
...
resource_time_dataframe.loc[len(resource_time_dataframe)] = [ecoli_pam.id, 'allocation coefficients',
                                                                 accuracy,time_elapsed, res_used_mb]
print(f'Done with calculating Flux Allocation Coefficients for {ecoli_pam}.\nTime:\t\t {time_elapsed} seconds \nSum:\t\t{accuracy}\n')
